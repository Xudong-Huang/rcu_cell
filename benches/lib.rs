...
#[bench]
fn read_write_epoch_2(b: &mut Bencher) {
    static REF: AtomicUsize = AtomicUsize::new(0);
    ...
    b.iter(|| {
        REF.store(0, Ordering::Relaxed);
        let rcu_cell = Arc::new(rcu_cell::epoch::RcuCell::new(Foo(42)));
        std::thread::scope(|s| {
            let rcu = rcu_cell.clone();
            s.spawn(move || {
                for i in 0..1000 {
...
#[bench]
fn read_write_seize_2(b: &mut Bencher) {
    static REF: AtomicUsize = AtomicUsize::new(0);
    ...
    b.iter(|| {
        REF.store(0, Ordering::Relaxed);
        let rcu_cell = Arc::new(rcu_cell::seize::RcuCell::new(Foo(42)));
        std::thread::scope(|s| {
            let rcu = rcu_cell.clone();
            s.spawn(move || {
                for i in 0..1000 {
